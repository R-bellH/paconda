...
class PRMGhost( GhostAgent ):
    ...
    def getDistribution( self, state):
        ...
        #TODO: add pacman position to the PRM
        ...
        best_actions = [action for action, distance in zip(legal_actions, distances_to_next_node) if distance == best_score]
        # Construct distribution
        dist = util.Counter()
    ...
    def sample_space(self, width,height,n):
        samples=[]
        for i in range(n):
             samples.append((round(random.uniform(0,width-1),3), round(random.uniform(0,height-1),3))) #round to 3 decimal places means a tolerance of 0.001
        return samples
