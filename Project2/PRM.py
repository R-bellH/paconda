...
# class probablistic_Road_mapping:
#     points = []
#     def sample_points(self, x,y,n):
#         """
#         Sample n points from a rectangle with width x and height y
#         """
#         self.points = [(random.uniform(0,x),random.uniform(0,y)) for i in range(n)]
#
#     def get_points(self):
#         return self.points
#
...
#         """
#         return min(self.points, key=lambda p: manhattanDistance(p, point))
...
from util import INF, get_pairs, merge_dicts, flatten, RED, default_selector
...
class PRM(Roadmap):
    ...
    def __init__(self, distance_fn, extend_fn, collision_fn, samples=[]):
        ...
        self.extend_fn = self.create_extend_fn(self.vertices) #extend_fn
...
##################################################
    def create_extend_fn(self,existing_vertices):
        def order_by_distance(q):
            '''
            return ordered list of vertices by distance to q
            '''
            return sorted(existing_vertices, key=lambda v: self.distance_fn(v.q, q))

        def extend_fn(v1, v2):
            path=[]
            if v1 not in existing_vertices: #TODO: existing_vertices is not a list of q, but a list of vertices (i think) so need to change this
                neighbors = order_by_distance(v1)
                for v in neighbors: # find the closest vertex in the existing vertices that have feasible path to v1
                    if not self.collision_fn(v.q, v1): # if there is no collision between v and v1 you can extend to it.
                        self.add([v1])
                        # path.append(self.extend_fn(v.q, v1))
                        break
            if v2 not in existing_vertices:
                neighbors = order_by_distance(v2)
                for v in neighbors:
                    if not self.collision_fn(v.q, v2):
                        self.add([v2])
                        # path.append(self.extend_fn(v.q, v2))
                        break
            # now we have v1 and v2 in the roadmap, we can connect them
            # use dijkstra and distance function to find the shortest path
            path = self(v1, v2)
            return path
        return extend_fn
...
def prm(start, goal, distance_fn, sample_fn, extend_fn, collision_fn,
        target_degree=4, connect_distance=INF, num_samples=100):
    """
